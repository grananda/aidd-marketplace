...
- **Real context:** review against `docs/detalle-historias-usuario.md` (US criteria), `docs/arquitectura-base.md` (patterns), and `docs/guia-estilos.md` (styles). **The three are optional, each on its own** — the trailing "if present" applies to all of them, not only to the last. Review with the ones that exist and say which were missing: a finding raised against a convention no document states is an opinion, and has to be presented as such.
