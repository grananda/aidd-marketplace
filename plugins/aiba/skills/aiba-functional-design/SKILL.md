...
- **Los scripts que te hagas van fuera del proyecto.** Si para resolver una incidencia o retocar un DF necesitas un script de Python de usar y tirar, escribelo en un directorio temporal (`mktemp -d`, o `/tmp/df-<algo>/`), **nunca en la carpeta del proyecto ni en `docs/`**. Lo que se queda ahi no lo limpia nadie: acaba versionado, confunde a quien abre el repo y no es un entregable. En la carpeta del proyecto solo van los `.docx` del DF. Si el script merece conservarse, di donde esta y por que, y deja que el humano decida si lo trae.
