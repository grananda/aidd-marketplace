...
# La atribucion vive en los scripts compartidos del plugin: la usan este skill
# y `aiba-metrics`, y dos copias serian dos definiciones de la misma causa.
...
def leer_auditoria(root: Path) -> tuple[dict, str | None]:
    ...
    for f in sorted(list(d.glob("*.jsonl")) + list(d.glob("*/*.jsonl"))):
        for linea in f.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            # dicen nada, y juntas con la duracion si. El plegado es compartido
            # para que `aiba-metrics` lea exactamente los mismos campos.
            if cid:
                acumular_senales(eventos.setdefault(cid, {}), e)

            for dec in e.get("decisions") or []:
                if (dec.get("type") == "bloqueante"
                        and str(dec.get("decision", "")).strip().lower() == "pendiente"):
                    # El contador por change lo lleva `acumular_senales`; aqui
                    # solo se recoge la lista, que es lo que este bucle aporta.
