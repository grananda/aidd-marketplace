...
- **En multirepo (`roadmap.multirepo: true`) un amend no sale de este repositorio.** Los changes de los demas lanes viven en otros repos, con su propio `openspec/` y su propio arbol: no puedes leerlos, ni enmendarlos, ni tomarles baseline. Si el delta afecta a otro repo, **enmienda solo la parte de este** y **di explicitamente que queda pendiente en cual** — el amend lo lanza ahi quien trabaje ese repo. No lo presentes como hecho.
