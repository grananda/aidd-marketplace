...
**Migrar de topologia lo hace el comando.** Si el usuario pide sacar `openspec/` del repo, partir el proyecto en varios o volver a meterlo, `aisdd roadmap` resuelve el destino con dos preguntas --cuantos repos tiene el codigo y si el registro vive dentro o en un repo aparte-- y **ejecuta la migracion**: inventario previo, mover las carpetas sin borrarlas del disco, los dos ficheros de exclusion --el `.gitignore` del repo de gobierno y el `.git/info/exclude` del de codigo, que no se versiona y por eso no deja rastro--, el `config.yaml` y el registro del cambio. Lo unico que deja al humano es lo irreversible: **reescribir la historia de git**, que cambia todos los hashes y no alcanza las copias que ya existan fuera.
