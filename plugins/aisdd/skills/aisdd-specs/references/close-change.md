...
3. **Comprueba que sigue en verde antes de archivar.** Ejecuta build y tests del repo, como hace `implement change`. **Archivar es promover los `spec.md` a linea base y dar el trabajo por entregado**, y entre implementar y cerrar puede haber pasado de todo: correcciones de nivel 2, un `amend`, el merge de otra rama. El unico verde registrado hasta ahora era el del momento de implementar, que puede ser de hace dias.

   - **Si pasa**, sigue y registra el resultado en el bloque `verification` de la entrada de auditoria.
   - **Si falla, detente y dilo.** Nombra que fallo y desde cuando: si el mismo test fallaba ya en `implement change` --mira su entrada de auditoria-- no es una regresion de este cierre, y decirlo cambia a quien le toca.
   - **El usuario puede archivar igualmente**, y a veces debe: un test roto por el entorno no puede bloquear un cierre cuyo codigo ya esta mergeado. Pero **la excepcion se declara**: pidesela explicitamente, registrala en `decisions.md` con `Tipo: correccion` y `Nivel: 2`, y **manda `status: partial`** en la auditoria con el fallo en `errors[]`. Un cierre en rojo con constancia es informacion; uno en rojo en silencio es una linea base que nadie sabe que esta sucia.
   - **Si no hay como ejecutarlos** --proyecto sin suite, o sin poder ejecutarla desde aqui--, dilo y sigue. No inventes un verde: omite el campo en vez de poner cero, que un cero se lee como cero fallos.

4. Ejecuta:
   ```bash
   openspec archive <change-slug>
   ```
5. **Transicion en Jira (opcional)**: si la integracion con Jira esta activa (ver "Integracion con Jira (opcional)" (`references/jira.md`)):
...
5b. **Solo en topologia `externalizado`: el archivado va despues del merge, y con varios repos son varias PR.** El `openspec/` esta en otro repositorio, asi que archivar aqui no espera a nadie: **el orden lo pones tu**. Archiva **despues** de que la PR del repo de codigo este mergeada, nunca antes.

    Y si el change toco **varios** repos --puede, porque el `openspec/` unico no lo impide--, no se entrega con un merge sino con tantos como repos toque. Archivarlo con dos abiertos deja el roadmap diciendo que la fase esta hecha cuando dos tercios del codigo no estan en ninguna rama principal, y **el informe de estado lo contara como avance real**.

    Antes de archivar, **por cada repo que el change toco** --uno o varios--, di si su trabajo esta integrado o cual es el PR pendiente. Si falta alguno, **no archives**: informa de cuales y para. El comando se relanza cuando esten.
...
8. **Escribe la entrada de auditoria.** Es obligatoria y **no es opcional para ningun comando salvo `aisdd lane`**. Componla con `audit.py` segun "Scripts del skill" (`references/scripts.md`), con el esquema y las reglas de "Auditoria y trazabilidad" (`references/audit.md`), y `prompt_version` = `<skill_version>:close-change`. Reporta despues su ruta y su `id` en la verificacion final.
   ...
   **Incluye el bloque `verification`** con lo que dieron el build y los tests del paso 3: `{build, tests_run, passed, failed, added, modified, gates[]}`. Hasta ahora `implement change` era el unico que lo registraba, asi que el ultimo verde conocido de un change podia ser de dias antes de archivarlo. Si no pudiste ejecutarlos, **omite el campo** en vez de poner cero.

   > **Este comando no verifica nada de diseno.** Eso lo hace el dev durante el desarrollo, o el validator; anadir aqui un juicio visual seria meterle a `close change` una decision que no le toca. Lo unico que puedes hacer, **a tu criterio y cuando lo veas pertinente**, es dejar constancia en `notes` de que fuente de diseno se uso --artefactos de `docs/design/` o `docs/guia-estilos.md`--. Es una nota discrecional, no un campo: **no sirve para medir despues** cuantos changes tiraron de cada fuente, y es el precio de no imponer ruido. **Y si `roadmap.topology` es `externalizado`, commitea y sube el repo de gobierno** antes de dar el comando por terminado: una entrada que solo existe en un portatil no es un registro. Ver "Ritmo de commit y push" (`references/governance-repo.md`).
