...
| Hay `sprint-plan.md` anterior **y el roadmap registro conflictos de alineacion** | `aiba sprint-planning` otra vez, **y esta vez no es opcional** | Alguna fase se partio o cambio de sprint, asi que hay HU comprometidas que ya no caen donde decia el plan. Nombra los conflictos: es lo que el negocio tiene que ver antes de decidir si re-empaqueta |
| Hay `sprint-plan.md` anterior, el roadmap cambio de modo **y no hubo conflictos** | Nada, y **dilo** | Los `change_hint` se conservaron, asi que el enlace con el sprint-plan y con Jira sigue en pie aunque las fases se llamen distinto. Re-ejecutar no aporta |
| Cualquiera | `booster-docs` sobre `docs/roadmap.md` | Vista HTML del roadmap, opcional |
| **`fraccionado`, y este es el repo donde se faseo** | **Copiar `docs/` a los demas repos**, y despues `aisdd init` + `aisdd roadmap` en cada uno | Va **el primero de todos**, antes que la capa de entrega: hasta que no se copie, los otros dos equipos no tienen roadmap y no lo saben. Nombra los repos y los ficheros; el comando no puede copiarlos porque no ve esos repositorios |
| **`fraccionado`, y este repo adopto el roadmap** | `aisdd open change` sobre la primera fase de este lane | Ya esta todo en su sitio. No sugieras copiar nada: aqui el documento llego, no salio |

Y siempre, como paso de ejecucion: **`aisdd open change`** (en `multilane`, precedido de `aisdd lane switch <lane-id>` si el lane activo no es el de la primera fase; **en topologia `fraccionado`, sin `switch`** — el lane es este repo, y si la primera fase es de otro, di en cual se abre y para).
...
| Topologia **`fraccionado`**, la siguiente fase de este repo | `aisdd open change` nombrandola. Si en este repo ya no quedan, **dilo y para ahi**: lo que falte se abre en su propio repositorio, y no es tuyo sugerirlo |
...
Que se puede hacer en el lane activo: `aisdd open change` si esta libre, `aisdd implement`/`close change <change-slug>` si tiene uno vivo, o las barreras que lo bloquean. **En topologia `fraccionado` no hay barreras** y el lane activo es siempre este repo. En `externalizado` las hay y el lane se elige como siempre.
