...
- **El numero de `multilane` es una cota optimista.** El script reparte fases por duracion; un corte de lanes real exige ademas **rutas y specs disjuntas**, que es un juicio de arquitectura. Si el corte no se sostiene (ver "Criterios de corte de lanes", `references/parallelism.md`), ese calendario no se alcanza. **Dilo siempre** al presentar un optimo `multilane`, y comprueba el corte en los pasos 1-7 de la construccion de lanes antes de darlo por bueno.
